...
class TestSubmissionCreation:
    ...
    def test_ocr_on_create_with_reddit_link(
        self, client: Client, settings: SettingsWrapper, setup_site: Any
    ) -> None:
        """Verify that a new submission completes the OCR process."""
        settings.ENABLE_OCR = True
        ...
        with patch(
            "api.models.process_image", return_value={"text": "hi, u/ToR!"}
        ) as mock:
            result = client.post(
                reverse("submission-list"),
                data,
        ...
        assert Transcription.objects.count() == 1
        transcription = Transcription.objects.first()
        assert transcription.text == "hi, \/u/ToR!"  # noqa: W605
        assert transcription.source == Source.objects.get(name="blossom")

    def test_ocr_on_create_with_shortlink(
        self, client: Client, settings: SettingsWrapper, setup_site: Any
    ) -> None:
        """Verify that a new submission completes the OCR process."""
        settings.ENABLE_OCR = True
        ...
        with patch(
            "api.models.process_image", return_value={"text": "https://aaa.com/aaaa"}
        ) as mock:
            result = client.post(
                reverse("submission-list"),
                data,
    ...
    def test_ocr_on_create_with_shortlink_and_reddit_ping(
        ...
        with patch(
            "api.models.process_image",
            return_value={"text": "https://aaa.com/aaaa -- it's the best, u/aa!"},
        ) as mock:
            result = client.post(
                reverse("submission-list"),
                data,
