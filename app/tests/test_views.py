...
from app.views import TranscribeSubmission
...
def test_unclaim_errors(client: Client, error: int, expected_redirect: str) -> None:
    ...
    with patch(
        "api.views.submission.SubmissionViewSet.unclaim", lambda a, b, c: response
    ):
        response = client.get(
            reverse("app_unclaim", kwargs={"submission_id": submission.id})
        )
...
class TestChooseSubmission:
    ...
    def test_check_reddit_for_missing_information(
        ...
        with patch("app.middleware.configure_reddit", lambda a: RedditSubmission()):
            submission = create_submission(
                original_id=int(random.random() * 1000), content_url="http://imgur.com",
            )
    ...
    def test_rank_up(self, client: Client) -> None:
        ...
        with patch("authentication.models.BlossomUser.ranked_up", True):
            response = client.get(reverse("choose_transcription"))
            assert response.context.get("show_confetti") is True

        with patch("authentication.models.BlossomUser.ranked_up", False):
            response = client.get(reverse("choose_transcription"))
...
class TestTranscribeSubmission:
    ...
    def test_get_errors(
        ...
        with patch(
            "api.views.submission.SubmissionViewSet.claim", lambda a, b, c: response
        ):
            response = client.get(
                reverse(
    ...
    def test_post_errors(
        ...
        with patch(
            "api.views.submission.SubmissionViewSet.done", lambda a, b, c: response
        ):
            response = client.post(
                reverse(
                    "transcribe_submission", kwargs={"submission_id": submission.id}
